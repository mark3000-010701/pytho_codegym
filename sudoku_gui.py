...
class Board:
        ...
        for i in range(9):
            for j in range(9):
                ...
                if self.tiles[i][j].value != 0:  # don't draw 0s on the grid
                    self.tiles[i][j].display(self.tiles[i][j].value, (20 + (j * 60), (5 + (i * 60))),
                                             (0, 0, 0))  # 20,5 are the coordinates of the first tile
        # bottom-most line
    ...
    def redraw(self, keys, wrong, time):
        ...
        if len(keys) != 0:  # draws inputs that the user places on board but not their final value on that tile
            for value in keys:
                self.tiles[value[0]][value[1]].display(keys[value], (20 + (value[0] * 60), (5 + (value[1] * 60))),
                                                       (128, 128, 128))
...
def main():
    ...
    text = font.render("wellcome game", True, (0, 0, 0))
    screen.blit(text, (12, 50))

    font = pygame.font.SysFont('Bahnschrift', 40)
    text = font.render("Soduku", True, (0, 0, 0))
    screen.blit(text, (30, 80))

    font = pygame.font.SysFont('Bahnschrift', 20)
    text = font.render("law of soduku:", True, (0, 0, 0))
    screen.blit(text, (20, 120))
    text = font.render("1.click mouse for fill board", True, (0, 0, 0))
    screen.blit(text, (20, 140))
    text = font.render("2.use keydown to fill board ", True, (0, 0, 0))
    screen.blit(text, (20, 160))
    text = font.render("3.keydown  'h' for hint", True, (0, 0, 0))
    screen.blit(text, (20, 180))
    text = font.render("4.keydown 'enter' for check answer", True, (0, 0, 0))
    screen.blit(text, (20, 200))
    text = font.render("5.keydown 'space' to auto fill answer", True, (0, 0, 0))
            ...
            if event.type == pygame.QUIT:
                ...
                if board.board[selected[1]][selected[0]] == 0 and selected != (-1, -1):
                    ...
                    if event.key == pygame.K_9:
                        ...
                        if selected in keyDict:
                            if keyDict[selected] != board.solvedBoard[selected[1]][
                                selected[0]]:  # clear tile when incorrect value is inputted
                                wrong += 1
                                board.tiles[selected[1]][selected[0]].value = 0
                                del keyDict[selected]
                                break
                            ...
                            board.tiles[selected[1]][selected[0]].value = keyDict[
                                selected]  # assigns current grid value
                            board.board[selected[1]][selected[0]] = keyDict[
                                selected]  # assigns to actual board so that the correct value can't be modified
                            del keyDict[selected]
